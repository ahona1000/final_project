...
# Step 1: Aggregate average salary by state from your dataset
# Replace 'State' and 'Salary' with the actual column names in your dataset
state_salary_data = gender_3.groupby('State')['Salary'].mean().reset_index()
state_salary_data.rename(columns={'Salary': 'avg_salary'}, inplace=True)

# Step 2: Load a GeoDataFrame of US states
# Using naturalearth_lowres as a general example; replace with a US-specific GeoJSON if needed
gdf = gpd.read_file(gpd.datasets.get_path('naturalearth_lowres'))
us_states_gdf = gdf[gdf['iso_a2'] == 'US']  # Filter only the United States

# Step 3: Merge GeoDataFrame with your salary data
us_salary_gdf = us_states_gdf.merge(state_salary_data, left_on='name', right_on='State', how='left')

# Step 4: Plot the map
fig, ax = plt.subplots(1, 1, figsize=(12, 8))
us_salary_gdf.plot(column='avg_salary', ax=ax, legend=True, cmap='coolwarm', 
...
gender_4 = pd.read_csv("/Users/ahonaroy/Documents/GitHub/final_project/job_title_des.csv")
...
```{python}
# Group by Job Title to see if certain job titles have more male or female words
job_title_grouped = gender_bias_df.groupby('Job Title').agg({'male_words': 'sum', 'female_words': 'sum'}).reset_index()

# Display the result to see the job titles and the corresponding male/female word counts
print(job_title_grouped)

# Optionally, plot the results to visualize the differences
import matplotlib.pyplot as plt
job_title_grouped.set_index('Job Title')[['male_words', 'female_words']].plot(kind='bar', figsize=(14, 7))
plt.title('Male vs Female Words in Job Titles')
...
# Assuming you've already loaded your dataset (gender_4.csv)
df = pd.read_csv('/Users/ahonaroy/Documents/GitHub/final_project/job_title_des.csv')

# Remove 'Unnamed: 0' column if it is just an index
df = df.drop(columns=['Unnamed: 0'])

# Define gendered words (you can expand these lists)
gendered_words = {
